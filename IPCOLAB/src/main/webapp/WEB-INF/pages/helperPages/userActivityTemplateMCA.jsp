...
<c:if test="${instanceModule.currActivity.activityOption.activity.activityTemplate.activityTemplateId eq 10}">
<div class="jumbotron">
	<div class="container-fluid text-center activity-content">
		<div class="container-fluid  mcaOptions form-group">
			<c:forEach items="${instanceModule.currActivity.answers}" var="answer">
			<!--  <div class="mcaOption <c:forEach items="${instanceModule.currActivity.activityOption.options}" var="option">${option.orderNo eq answer.orderNo 
					and option.isCorrect eq 'true'?'showAnswer':''}</c:forEach>" id="mcaOption_${answer.optionId}">-->
			<div class="mcaOption <c:forEach items="${instanceModule.currActivity.activityOption.options}" var="option"><c:if test="${option.orderNo eq answer.orderNo 
					and option.isCorrect eq true}">showAnswer</c:if><c:if test="${option.orderNo eq answer.orderNo 
					and option.isCorrect eq false}">showIncorrectAnswer</c:if></c:forEach>" id="mcaOption_${answer.optionId}">		
