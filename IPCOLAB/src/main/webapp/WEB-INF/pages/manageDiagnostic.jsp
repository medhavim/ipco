...
<html lang="en">
<head>
<title>Mangage Diagnostic</title>
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="jumbotron category_holder">
					...
					<div class="row">
						<div class="col-sm-12">
						<span class="h2">No categories available. You might want to add the first one.</span>
						</div>
					</div>
					</c:if>
				</div>
				...
				<c:forEach items="${allCategories}" var="category" varStatus="categoryNo">
					<div class="jumbotron category_holder">
						<div class="row">
							<div class="col-sm-6">
							<span class="container_name h2 collapsed span-category-${category.categoryId}" 
							...
							<div class="col-sm-2">
							<button class="btn btn-info btn-block" data-toggle="collapse" data-target="#viewCategoryDesc_${category.categoryId}">View Description</button>
							</div>
							<div class="col-sm-2">
							<button class="btn btn-warning btn-block" id="${category.categoryId}" name="${category.categoryTitle}" onclick="renameCategory(this)">Rename</button>
							</div>
							<div class="col-sm-2">
							<a class="btn btn-danger btn-block" id="deleteId_${category.categoryId}" role="button" onclick="deleteCategory(this)">Delete</a>
							</div>
						...
						<div class="panel-collapse collapse" id="viewCategoryDesc_${category.categoryId}">
							<div class="container-fluid categoryDescContainer_${category.categoryId}">
								...
								<div class="row"><div class="col-sm-2 pull-right">
									<button class="btn btn-warning btn-block form-control" onclick="editCategoryDesc(${category.categoryId})">Edit</button>
								</div></div>
							</div>
							<div class="container-fluid form-group categoryDescEditor_${category.categoryId} desc-body" style="display: none">
								...
								<div class="col-sm-2 pull-right desc-body">
									<button class="btn btn-success btn-block" onclick="updateCategoryDesc(${category.categoryId})">Update</button>
								</div>
								</div>
							</div><hr>
						</div>
					
						<div class="panel-collapse collapse" id="diagnostics_for-${category.categoryId}">
							<div class="jumbotron diagnostic_holder">
								<div class="row">
									<div class="col-sm-4">
										<form action="gotoAddDiagnostic.action" method="post">
											<input type="hidden" name="categoryId" value="${category.categoryId}"/>
											<input type="submit" class="btn btn-info btn-block" role="button" value="Add a Diagnostic Question"/>
										</form>
									</div>
								</div>
							...
							<div class="jumbotron diagnostic_holder">
								<div class="row">
									...
									<div class="col-sm-2">
									<button class="btn btn-warning btn-block" id="editDiagnostic_${diagnostic.diagnosticId}" name="${diagnostic.activity.activityTitle}" onclick="editDiagnostic(this)">Edit</button>
									</div>
									<div class="col-sm-2">
									<a class="btn btn-danger btn-block" id="deleteId_${diagnostic.diagnosticId}" role="button" onclick="deleteDiagnostic(this)">Delete</a>
									</div>
								</div>
								...
								<div class="panel-collapse collapse" id="topics_for-${diagnostic.diagnosticId}">
									<div class="jumbotron activity_holder">
										<div class="container-fluid">
											<div class="desc-body">${diagnostic.activity.activityText}</div>
										</div>
										<div class="row">
										<c:if test="${fn:length(diagnostic.topics)>0}">
										<c:forEach items="${diagnostic.topics}" var="topic" varStatus="topicNo">
											<div class="col-sm-3">
												<a class="btn btn-info btn-block" role="button">${topic.topicName}</a>
											</div>
										</c:forEach>
										</c:if>
										</div>
